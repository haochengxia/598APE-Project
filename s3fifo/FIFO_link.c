...
// in this version, we simply walk through the list to find the position
...
static cache_obj_t *FIFO_find(cache_t *cache, const request_t *req,
  ...
  if (update_cache) { 
    ...
    // if hit, go through the queue to get the position
    FIFO_params_t *params = (FIFO_params_t *)cache->eviction_params;
    cache_obj_t *cur = params->q_head;
    uint64_t cur_pos = 0;
    while (cur != params->q_tail) {
      if (cur->obj_id == obj->obj_id) {
        // update the position
        params->hit_pos[cur_pos]++;
        break;
      }
      cur = cur->queue.next;
      cur_pos++;
