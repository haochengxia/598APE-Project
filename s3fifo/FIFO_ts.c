...
static bool FIFO_remove(cache_t *cache, const obj_id_t obj_id) {
  ...
  if (params->has_obj_removed) {
    for (uint64_t i = 0; i < params->removed_track.n_removed; i++) {
      uint64_t removed_ts = params->removed_track.removed_timestamps[i];
      if (removed_ts > obj->logic_timestamp && removed_ts < params->logic_timer) {
        n_removed_in_range++;
      }
    }
  }
