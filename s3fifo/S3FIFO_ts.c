...
typedef struct {
  ...
  // arrays to store the positions of the objects in the main, small, and ghost FIFOs
  uint64_t *main_hit_pos;
  uint64_t *small_hit_pos;
  uint64_t *ghost_hit_pos;
...
cache_t *S3FIFO_init(const common_cache_params_t ccache_params, const char *cache_specific_params) {
  ...
  // init arrays to store the positions of the objects in the main, small, and ghost FIFOs
  params->main_hit_pos = (uint64_t*)calloc(main_fifo_size + 16, sizeof(uint64_t));
  params->small_hit_pos = (uint64_t*)calloc(small_fifo_size + 16, sizeof(uint64_t));
  params->ghost_hit_pos = (uint64_t*)calloc(ghost_fifo_size + 16, sizeof(uint64_t));
  memset(params->main_hit_pos, 0, sizeof(uint64_t) * main_fifo_size);
  memset(params->small_hit_pos, 0, sizeof(uint64_t) * small_fifo_size);
  memset(params->ghost_hit_pos, 0, sizeof(uint64_t) * ghost_fifo_size);

  common_cache_params_t ccache_params_local = ccache_params;
  ccache_params_local.cache_size = small_fifo_size;
  // params->small_fifo = FIFO_init(ccache_params_local, NULL);
  // pass the small_hit_pos to FIFO_init to count inner small queue hits
  params->small_fifo = FIFO_init(ccache_params_local, (char*)params->small_hit_pos);
  ...
  if (ghost_fifo_size > 0) {
    ...
    // pass the ghost_hit_pos to FIFO_init to count inner ghost queue hits
    params->ghost_fifo = FIFO_init(ccache_params_local, (char*)params->ghost_hit_pos);
  ...
  // pass the main_hit_pos to FIFO_init to count inner main queue hits
  params->main_fifo = FIFO_init(ccache_params_local, (char*)params->main_hit_pos);
...
static void S3FIFO_free(cache_t *cache) {
  ...
  printf("Total time to run simulation %0.6f seconds\n", 
         tdiff(&start, &end));
  // dump report
  printf("S3FIFO report:\n");
  
  // Calculate total hits and queue sizes
  cache_t *small = params->small_fifo;
  cache_t *main = params->main_fifo;
  cache_t *ghost = params->ghost_fifo;
  
  int64_t small_size = small->cache_size;
  int64_t main_size = main->cache_size;
  int64_t ghost_size = ghost ? ghost->cache_size : 0;
  
  // Count total hits in each queue from the hit position arrays
  uint64_t small_hits = 0, main_hits = 0, ghost_hits = 0;
  for (int64_t i = 0; i < small_size; i++) {
    small_hits += params->small_hit_pos[i];
  }
  for (int64_t i = 0; i < main_size; i++) {
    main_hits += params->main_hit_pos[i];
  }
  if (ghost) {
    for (int64_t i = 0; i < ghost_size; i++) {
      ghost_hits += params->ghost_hit_pos[i];
    }
  }
  
  uint64_t total_hits = small_hits + main_hits + ghost_hits;
  if (total_hits == 0) {
    printf("  No hits recorded\n");
    return;
  }
  
  printf("  Total hits: %lu (small: %lu (%.2f%%), main: %lu (%.2f%%), ghost: %lu (%.2f%%))\n", 
         total_hits, 
         small_hits, (double)small_hits/total_hits*100,
         main_hits, (double)main_hits/total_hits*100,
         ghost_hits, (double)ghost_hits/total_hits*100);
  
  // Create 10 bins for each queue and report distribution
  int num_bins = 10;

  // Small queue bin distribution
  if (small_size > 0) {
    printf("  Small queue hit distribution (%.2f%% of total hits):\n", (double)small_hits/total_hits*100);
    int64_t bin_size = (small_size + num_bins - 1) / num_bins; // ceiling division
    for (int bin = 0; bin < num_bins; bin++) {
      int64_t bin_start = bin * bin_size;
      int64_t bin_end = MIN((bin + 1) * bin_size - 1, small_size - 1);
      if (bin_start > small_size - 1) break;
      
      uint64_t bin_hits = 0;
      for (int64_t i = bin_start; i <= bin_end; i++) {
        bin_hits += params->small_hit_pos[i];
      }
      
      printf("    Bin %d [pos %ld-%ld]: %lu hits (%.2f%% of small, %.2f%% of total)\n", 
             bin, bin_start, bin_end, bin_hits, 
             small_hits > 0 ? (double)bin_hits/small_hits*100 : 0, 
             (double)bin_hits/total_hits*100);
    }
  }
  
  // Main queue bin distribution
  if (main_size > 0) {
    printf("  Main queue hit distribution (%.2f%% of total hits):\n", (double)main_hits/total_hits*100);
    int64_t bin_size = (main_size + num_bins - 1) / num_bins; // ceiling division
    for (int bin = 0; bin < num_bins; bin++) {
      int64_t bin_start = bin * bin_size;
      int64_t bin_end = MIN((bin + 1) * bin_size - 1, main_size - 1);
      if (bin_start > main_size - 1) break;
      
      uint64_t bin_hits = 0;
      for (int64_t i = bin_start; i <= bin_end; i++) {
        bin_hits += params->main_hit_pos[i];
      }
      
      printf("    Bin %d [pos %ld-%ld]: %lu hits (%.2f%% of main, %.2f%% of total)\n", 
             bin, bin_start, bin_end, bin_hits, 
             main_hits > 0 ? (double)bin_hits/main_hits*100 : 0, 
             (double)bin_hits/total_hits*100);
    }
  }
  
  // Ghost queue bin distribution
  if (ghost_size > 0 && ghost_hits > 0) {
    printf("  Ghost queue hit distribution (%.2f%% of total hits):\n", (double)ghost_hits/total_hits*100);
    int64_t bin_size = (ghost_size + num_bins - 1) / num_bins; // ceiling division
    for (int bin = 0; bin < num_bins; bin++) {
      int64_t bin_start = bin * bin_size;
      int64_t bin_end = MIN((bin + 1) * bin_size - 1, ghost_size - 1);
      if (bin_start > ghost_size - 1) break;
      
      uint64_t bin_hits = 0;
      for (int64_t i = bin_start; i <= bin_end; i++) {
        bin_hits += params->ghost_hit_pos[i];
      }
      
      printf("    Bin %d [pos %ld-%ld]: %lu hits (%.2f%% of ghost, %.2f%% of total)\n", 
             bin, bin_start, bin_end, bin_hits, 
             ghost_hits > 0 ? (double)bin_hits/ghost_hits*100 : 0, 
             (double)bin_hits/total_hits*100);
    }
  }
  ...
  free(params->main_hit_pos);
  free(params->small_hit_pos);
  free(params->ghost_hit_pos);
